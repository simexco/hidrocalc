...
interface Props {
  ...
  hidePerdidas?: boolean  // ocultar la tabla de pérdidas por accesorio (el despiece la muestra consolidada al final)
...
export default function ListaMaterialesSIMEX({
  dnMM, dnStr, materialRaw, hf, longitud, onHmChange, mode = 'full', externalAccs, onAccsChange, externalConex, onConexChange, readOnly = false, hidePerdidas = false, onDelete, onComputed
}: Props) {
      ...
      {!readOnly && piezasPrinc.filter(a=>!a.isWafer&&!a.isObra&&(a.bridas>0||(a.bridas2??0)>0)).length >= 2 && (
        <div className="rounded-xl border border-[#1C3D5A]/25 bg-[#1C3D5A]/[0.03] dark:bg-gray-800/40 p-4 space-y-2">
          <div className="flex items-center gap-2">
            <svg viewBox="0 0 24 24" className="w-4 h-4 text-[#1C3D5A] dark:text-blue-300" fill="none" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round"><path d="M9 12h6M7 8a4 4 0 0 0 0 8h2M17 8a4 4 0 0 1 0 8h-2" /></svg>
  ...
  if (mode === 'selector') return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 space-y-4">
      ...
      {sugEnterrada && (<div onClick={addMarco} className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-xs text-yellow-700 cursor-pointer hover:bg-yellow-100"><strong>Sugerencia:</strong> ¿Válvula enterrada? → Agregar Marco con Tapa</div>)}
    </div>
  )
  ...
  return (
    <div className="mt-6 pt-5 border-t border-gray-200 dark:border-gray-700">
      ...
      {sugEnterrada && (<div onClick={addMarco} className="p-3 bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 rounded-lg mb-3 text-xs text-yellow-700 cursor-pointer hover:bg-yellow-100 transition-colors"><strong>Sugerencia:</strong> ¿Esta válvula va enterrada? → Clic para agregar Marco con Tapa Dúctil AI-MCT-D</div>)}
