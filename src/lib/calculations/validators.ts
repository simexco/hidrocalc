...
export function getVelocityAlerts(V: number | null): Alert[] {
  ...
  if (V < VELOCITY_LIMITS.sedimentationRisk) {
    alerts.push({ level: "ERROR", field: "V", message: "Velocidad minima no alcanzada (V < 0.3 m/s) — riesgo sedimentacion. Hazen-Williams pierde precision a flujos laminares, verificar con Darcy-Weisbach." });
    ...
    alerts.push({ level: "ERROR", field: "V", message: "Velocidad excesiva (V > 3.5 m/s) — erosion y golpe de ariete" });
  }
...
export function getPressureAlerts(P2: number | null): Alert[] {
  ...
  if (P2 < PRESSURE_LIMITS.negative) {
    alerts.push({ level: "CRITICAL", field: "P2", message: "Presion negativa — posible cavitacion" });
  } else if (P2 < PRESSURE_LIMITS.minCONAGUA) {
    alerts.push({ level: "ERROR", field: "P2", message: "Presion insuficiente (min. CONAGUA: 1.0 kg/cm2 = 10 m.c.a.)" });
  } else if (P2 <= PRESSURE_LIMITS.maxNormal) {
    alerts.push({ level: "OK", field: "P2", message: "Presion dentro de rango normativo" });
  } else if (P2 <= PRESSURE_LIMITS.excessive) {
    alerts.push({ level: "WARN", field: "P2", message: "Presion elevada — considerar valvula reductora" });
  } else {
    alerts.push({ level: "ERROR", field: "P2", message: "Presion excesiva — revisar diseno" });
  }
...
export function getGradientAlerts(J_km: number | null): Alert[] {
  ...
  if (J_km < GRADIENT_LIMITS.low) {
    ...
    alerts.push({ level: "OK", field: "J", message: "Gradiente optimo (J <= 5 m/km) — Ref. NOM-001-CONAGUA" });
    ...
    alerts.push({ level: "ERROR", field: "J", message: "Gradiente alto (J > 10 m/km) — revisar diametro. Ref. NOM-001-CONAGUA: J optimo < 5, maximo 10 m/km" });
  }
